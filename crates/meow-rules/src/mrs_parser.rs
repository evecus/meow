...
pub const RMRS_MAGIC: [u8; 4] = *b"RMRS";
...
pub fn parse_header(data: &[u8]) -> Result<(MrsHeader, &[u8]), MrsError> {
    ...
    if data[..4] != MRS_MAGIC && data[..4] != RMRS_MAGIC {
        return Err(MrsError::WrongFormat);
    }
...
mod tests {
    ...
    #[test]
    fn rmrs_header_is_accepted() {
        let bytes = write_ruleset_rmrs(TYPE_DOMAIN, &["example.com"]).unwrap();
        let (hdr, _rest) = parse_header(&bytes).unwrap();
        assert_eq!(hdr.type_tag, TYPE_DOMAIN);
...
/// Write a complete rmrs rule-set file (header + zstd-compressed payload).
/// rmrs is meow-rs specific and intentionally mrs-compatible except for magic.
pub fn write_ruleset_rmrs(type_tag: u8, entries: &[&str]) -> Result<Vec<u8>, MrsError> {
    let mut out = write_ruleset_mrs(type_tag, entries)?;
    out[..4].copy_from_slice(&RMRS_MAGIC);
    Ok(out)
}
