...
pub trait RuleSet: Send + Sync {
    ...
    /// Return the raw string entries that constitute this rule set.
    ///
    /// For `Domain` and `Classical` sets these are the original domain/rule
    /// strings. For `IpCidr` sets these are CIDR strings (`"10.0.0.0/8"`).
    ///
    /// Used by the API `/providers/rules/{name}/download` endpoint to
    /// serialise the live snapshot back to `.rmrs` binary format without
    /// needing to cache the original bytes.
    fn entries(&self) -> Vec<String> {
        vec![] // default: empty — concrete types override
    }

    // Convenience aliases kept for call-site clarity in the API handler.
    fn domain_entries(&self) -> Vec<String> { self.entries() }
    fn ipcidr_entries(&self) -> Vec<String> { self.entries() }
    fn classical_entries(&self) -> Vec<String> { self.entries() }
}
...
/// Return `true` if `bytes` starts with the MRS/RMRS magic.
pub fn is_mrs_bytes(bytes: &[u8]) -> bool {
    bytes.len() >= 4
        && (bytes[..4] == crate::mrs_parser::MRS_MAGIC
            || bytes[..4] == crate::mrs_parser::RMRS_MAGIC)
}
...
pub struct DomainRuleSet {
    trie: DomainTrie<()>,
    count: usize,
    /// Original entry strings preserved for round-trip serialisation.
...
impl RuleSet for IpCidrRuleSet {
    ...
    fn entries(&self) -> Vec<String> {
        self.cidrs.iter().map(|n| n.to_string()).collect()
    }
}
...
pub struct ClassicalRuleSet {
    ...
    /// Original rule strings preserved for round-trip serialisation.
    raw_entries: Vec<String>,
}
