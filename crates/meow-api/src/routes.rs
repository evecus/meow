...
pub fn create_router(state: Arc<AppState>) -> Router {
    ...
    let api = Router::new()
        ...
        .route(
            "/providers/rules/{name}/download",
            get(download_rule_provider_rmrs),
        )
...
// ── Rule provider binary download ──────────────────────────────────────

/// `GET /providers/rules/{name}/download`
///
/// Returns the current snapshot of the named rule provider encoded as an
/// `.rmrs` binary (meow-rs native format, MRS-compatible except for the
/// `RMRS` magic prefix).  The response has Content-Type
/// `application/octet-stream` and a `Content-Disposition` header
/// suggesting `<name>.rmrs` as the filename.
///
/// Useful for:
/// - exporting a provider snapshot for offline use.
/// - seeding a remote `meow` instance with the same rule set without
///   requiring the remote to re-fetch from the original HTTP URL.
async fn download_rule_provider_rmrs(
    State(state): State<Arc<AppState>>,
    Path(name): Path<String>,
) -> Result<impl axum::response::IntoResponse, StatusCode> {
    use axum::http::header;
    use meow_rules::{
        mrs_parser::{write_ruleset_rmrs, TYPE_CLASSICAL, TYPE_DOMAIN, TYPE_IPCIDR},
        RuleSetBehavior,
    };

    let provider = {
        let providers = state.rule_providers.read();
        providers.get(&name).cloned().ok_or(StatusCode::NOT_FOUND)?
    };

    let behavior = provider.behavior;
    let ruleset_snap = provider.snapshot();

    // Extract the rule entries from the live rule set snapshot.
    // We re-encode from the in-memory representation rather than caching
    // raw bytes, so the download always reflects the most recent state.
    let (type_tag, entries) = match behavior {
        RuleSetBehavior::Domain => {
            let entries = ruleset_snap.domain_entries();
            (TYPE_DOMAIN, entries)
        }
        RuleSetBehavior::IpCidr => {
            let entries = ruleset_snap.ipcidr_entries();
            (TYPE_IPCIDR, entries)
        }
        RuleSetBehavior::Classical => {
            let entries = ruleset_snap.classical_entries();
            (TYPE_CLASSICAL, entries)
        }
    };

    let entry_refs: Vec<&str> = entries.iter().map(String::as_str).collect();
    let bytes = write_ruleset_rmrs(type_tag, &entry_refs).map_err(|e| {
        tracing::warn!(provider = %name, "rmrs encode failed: {e}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let filename = format!("{name}.rmrs");
    let response = (
        [
            (
                header::CONTENT_TYPE,
