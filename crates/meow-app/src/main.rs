...
enum Command {
    ...
    /// Show service status
    Status,
    /// Convert a Clash/Mihomo rule-provider YAML file into meow rmrs binary format
    BuildRmrs {
        /// Input yaml file containing payload list
        #[arg(short = 'i', long = "input")]
        input: String,
        /// Output rmrs file path
        #[arg(short = 'o', long = "output")]
        output: String,
        /// Rule behavior type
        #[arg(short = 'b', long = "behavior", value_enum, default_value = "domain")]
        behavior: RulesetType,
    },
}
...
fn handle_service_command(cmd: &Command, args: &Args) -> Result<()> {
    match cmd {
        ...
        Command::BuildRmrs {
            ...
            output,
            behavior,
        } => build_rmrs(input, output, *behavior),
    }
}
...
fn build_rmrs(input: &str, output: &str, behavior: RulesetType) -> Result<()> {
    let raw = std::fs::read_to_string(input)?;
    let root: serde_yaml::Value = serde_yaml::from_str(&raw)?;
    let payload = root
        .get("payload")
        .and_then(|v| v.as_sequence())
        .ok_or_else(|| anyhow::anyhow!("yaml missing payload array"))?;
    let entries: Vec<&str> = payload.iter().filter_map(|v| v.as_str()).collect();
    if entries.len() != payload.len() {
        return Err(anyhow::anyhow!("payload must contain only strings"));
    }
    let type_tag = match behavior {
        RulesetType::Domain => meow_rules::mrs_parser::TYPE_DOMAIN,
        RulesetType::Ipcidr => meow_rules::mrs_parser::TYPE_IPCIDR,
        RulesetType::Classical => meow_rules::mrs_parser::TYPE_CLASSICAL,
    };
    let bytes = meow_rules::mrs_parser::write_ruleset_rmrs(type_tag, &entries)
        .map_err(|e| anyhow::anyhow!(e.to_string()))?;
    std::fs::write(output, bytes)?;
    println!("written rmrs: {output}");
    Ok(())
}
