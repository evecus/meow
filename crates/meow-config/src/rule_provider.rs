...
//! and `mrs`/`rmrs` formats (auto-detected by magic bytes for http/file).
...
fn parse_bytes_to_ruleset_with_format(
    ...
    let use_mrs = matches!(
        explicit_format,
        Some(RuleSetFormat::Mrs | RuleSetFormat::Rmrs)
    ) || is_mrs_bytes(bytes);
    if use_mrs {
    ...
    let entries = match explicit_format.unwrap_or(RuleSetFormat::Yaml) {
        ...
        RuleSetFormat::Mrs | RuleSetFormat::Rmrs => unreachable!("handled above"),
    };
