...
///
/// Full field parity with mihomo `adapter/outbound/hysteria2.go`:
///
/// | Field              | Notes                                                          |
/// |--------------------|----------------------------------------------------------------|
/// | `server`           | required                                                       |
/// | `port`             | required, non-zero                                             |
/// | `password`         | required, non-empty                                            |
/// | `sni`              | TLS SNI override; defaults to `server`                         |
/// | `skip-cert-verify` | bool; disables TLS cert validation (insecure)                  |
/// | `ca`               | Path to PEM CA bundle file                                     |
/// | `ca-str`           | Inline PEM CA bundle string                                    |
/// | `obfs`             | Obfuscation type; only `"salamander"` is supported             |
/// | `obfs-password`    | Required when `obfs = "salamander"`                            |
/// | `up`               | Upload bandwidth hint e.g. `"100 mbps"` or bare Mbps integer   |
...
/// - `port == 0`.
/// - empty `password`.
/// - `obfs` is set but `obfs-password` is absent/empty.
/// - unknown `obfs` type (only `"salamander"` is valid).
/// - `ca` file path cannot be read.
///
...
fn parse_hysteria2(
    ...
    // Custom CA certificate (file path takes precedence over inline string).
    let ca_pem: Option<Vec<u8>> = if let Some(ca_path) = config.get("ca").and_then(|v| v.as_str()) {
        let bytes = std::fs::read(ca_path)
            .map_err(|e| format!("hysteria2[{name}]: failed to read ca file '{ca_path}': {e}"))?;
        Some(bytes)
    } else if let Some(ca_str) = config.get("ca-str").and_then(|v| v.as_str()) {
        Some(ca_str.as_bytes().to_vec())
    } else {
        None
    };

    // Obfuscation settings.
    let obfs_type = config.get("obfs").and_then(|v| v.as_str());
    let obfs_password = config.get("obfs-password").and_then(|v| v.as_str());

    // Bandwidth hints (advisory; stored but not yet enforced at QUIC layer).
    let up_bps = config.get("up").and_then(|v| {
        v.as_str()
            .and_then(meow_proxy::hysteria2_adapter::parse_bandwidth_bps)
            .or_else(|| v.as_u64().map(|n| n * 1_000_000))
    });
    let down_bps = config.get("down").and_then(|v| {
        v.as_str()
            .and_then(meow_proxy::hysteria2_adapter::parse_bandwidth_bps)
            .or_else(|| v.as_u64().map(|n| n * 1_000_000))
    });

    meow_proxy::Hy2Adapter::new_full(
        name,
        server,
        port,
        password,
...
"#,
    ...
    // ─── hysteria2 extended field tests ─────────────────────────────────────

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_salamander_obfs_ok() {
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             obfs: salamander\nobfs-password: obfspwd\n",
        );
        assert!(parse_proxy(&cfg).is_ok(), "salamander obfs should parse ok");
    }

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_salamander_missing_password_errors() {
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             obfs: salamander\n",
        );
        let Err(err) = parse_proxy(&cfg) else {
            panic!("must hard-error when obfs-password absent");
        };
        assert!(err.contains("obfs-password"), "msg: {err}");
    }

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_unknown_obfs_errors() {
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             obfs: fake-type\nobfs-password: pwd\n",
        );
        let Err(err) = parse_proxy(&cfg) else {
            panic!("unknown obfs type must hard-error");
        };
        assert!(err.contains("unsupported obfs type"), "msg: {err}");
    }

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_bandwidth_string_parses() {
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             up: \'100 mbps\'\ndown: \'200 mbps\'\n",
        );
        assert!(parse_proxy(&cfg).is_ok(), "bandwidth strings should parse ok");
    }

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_bandwidth_bare_int_parses() {
        // bare integer treated as Mbps (upstream compat)
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             up: 50\ndown: 100\n",
        );
        assert!(parse_proxy(&cfg).is_ok(), "bare int bandwidth should parse ok");
    }

    #[cfg(feature = "hysteria2")]
    #[test]
    fn parse_hysteria2_ca_str_parses() {
        // ca-str with a bogus PEM — parser must accept it syntactically
        // (TLS failure happens at connection time, not parse time).
        let cfg = hy2_config(
            "name: x\ntype: hysteria2\nserver: 1.2.3.4\nport: 443\npassword: pw\n             ca-str: \'not-a-real-cert\'\n",
        );
        // ca-str with unparseable PEM: accepted at config parse level
        // (build_quic_client_config will reject at connect time).
        // parse_proxy itself should succeed — it only reads the string.
        assert!(parse_proxy(&cfg).is_ok(), "ca-str should parse ok syntactically");
    }
